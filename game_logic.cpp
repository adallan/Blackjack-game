...
public:
    ...
    //Uses a random seed to shuffle the deck
    void shuffle ()
    {
    ...
    //Deals a card, will be used a lot to add cards into player and dealer hands.
    Card deal_card()
    {
        if (card_deck.size() == 0)
        {
...
public:
    ...
    {
        //ensures players base hand is empty
        hand.clear();
    }
    //Adds cards into the player hand
    ...
    //used in betting and chip logic
...
//Used in hit/stay function to chekc if player has bust every time they add another card into their hand.
...
{
    //Uses a while loop with a sentinel to iteratively hit more cards while checking for busts.
...
{
    //Check who won the hand and adjust player's chips.
...
{
    // Creates a new deck and shuffles it every hand and deals out two cards to each the player and dealer.
    deck.new_deck();
    deck.shuffle();
    player.add_card(deck.deal_card());
...
void dealer_turn(Dealer& dealer, Deck& deck)
{
    ...
    while (dealer.get_hand_value() < 17)
    {
        ...
        {
            //Checks if dealer busts to end to loop, otherwise dealer will stay with a busted hand.
            dealer.print_hand();
...
void game_on()
{
    //Base game logic and running function.
    Deck deck;
    Player player1(20);
    Dealer dealer;
    int prize_pool = 0;
